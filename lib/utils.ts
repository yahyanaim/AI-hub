    ...
    ${course.roadmap.map((step, i) => {
      ...
      <div style="display:flex;gap:16px;padding:18px 0;${i > 0 ? 'border-top:1px solid #f1f5f9' : ''}">
        <div style="display:flex;flex-direction:column;align-items:center;flex-shrink:0">
          <div style="width:28px;height:28px;border-radius:50%;background:${isFirst ? '#FF6B00' : '#f1f5f9'};color:${isFirst ? '#fff' : '#64748b'};font-size:12px;font-weight:700;line-height:28px;text-align:center">${i + 1}</div>
          ${i < course.roadmap.length - 1 ? `<div style="width:2px;flex:1;min-height:20px;background:#e2e8f0;margin:4px auto"></div>` : ''}
        </div>
