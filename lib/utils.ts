...
/**
 * Download an AI-skill entry as a SKILL.md file (Claude Code skill format),
 * generated client-side from the seed data.
 */
export function downloadSkillMd(tool: {
  name: string
  url: string
  description?: string
  tagline?: string
}): void {
  const path = tool.url.includes('/skill/') ? tool.url.split('/skill/')[1] : ''
  const slug = tool.url.split('/').filter(Boolean).pop() ?? 'skill'
  ...
  const md = [
    ...
    '',
    '```bash',
    `npx claude-code-templates@latest --skill ${path}`,
    '```',
    '',
    '## Source',
    '',
  ...
  a.download = `${path ? path.split('/').pop() : slug}.md`
