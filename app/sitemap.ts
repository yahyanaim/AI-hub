...
import { SEED_TOOLS, SEED_DEV_TOOLS, SEED_REPOS, SEED_USERS } from '@/lib/seed'
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticPages = [
    ...
    { url: `${baseUrl}/prompts`, lastModified: new Date(), changeFrequency: 'daily' as const, priority: 0.9 },
    { url: `${baseUrl}/repos`, lastModified: new Date(), changeFrequency: 'daily' as const, priority: 0.9 },
  ...
  const promptPages = SEED_DEV_TOOLS.map((tool) => ({
    url: `${baseUrl}/prompts/${tool.slug}`,
    lastModified: new Date(tool.updatedAt),
    changeFrequency: 'weekly' as const,
    priority: 0.7,
  }))
  ...
  return [...staticPages, ...toolPages, ...promptPages, ...repoPages, ...userPages]
}
